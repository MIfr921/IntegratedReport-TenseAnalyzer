...
# ===== フォント設定（Cloud対応：Google NotoフォントをDL） =====
...
if uploaded_file is not None:
    ...
    # ローマ字版（内部・CSV用）
    df_end["Ending_Romaji"] = df_end["語尾"].apply(to_roman)

    # ===== 📊 文末語尾頻度（グラフは日本語） =====
    st.subheader("📊 文末語尾の出現頻度（日本語表示）")
    st.dataframe(df_end[["Ending_Romaji","出現回数"]], use_container_width=True)

    fig1, ax1 = plt.subplots(figsize=(6,4))
    ax1.barh(df_end["語尾"], df_end["出現回数"], color="steelblue")
    ax1.invert_yaxis()
    ...
    # ===== 🕰 時制別頻出語 =====
    st.subheader("🕰 時制別頻出語（上位20語、グラフは日本語）")
    tokenizer = Tokenizer()
    def extract_words(t):
        words=[]
    ...
    with col1:
        ...
        if not past_df.empty:
            ...
            ax2.invert_yaxis();ax2.set_title("過去形：頻出単語")
            st.pyplot(fig2)
    with col2:
        ...
        if not fut_df.empty:
            ...
            ax3.barh(fut_df["単語"],fut_df["出現回数"],color="orange")
            ax3.invert_yaxis();ax3.set_title("現在・未来形：頻出単語")
            st.pyplot(fig3)

    # ===== CSV出力（ローマ字データ） =====
    csv = df_end[["Ending_Romaji","出現回数"]].to_csv(index=False).encode('utf-8-sig')
    st.download_button(
        "📥 文末語尾集計結果をCSVでダウンロード（ローマ字）",
        data=csv,
        file_name="ending_counts_romaji.csv",
        mime="text/csv"
    )
